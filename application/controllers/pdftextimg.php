...
function get_text()
{
    ...
    shell_exec("pdfimages -j ".$path." ".$imgf);
    $pdf = new pdf( $path );
    $n=0;
    $pages = $pdf->get_pages();
    ...
    while( list($nr,$page) = each($pages) )
    {
        ...
        $text = $page->get_text();
        $position_num = strpos($text,"Class");
        $result_string = substr("$text",$position_num+6,2);
        $position_num1 = strpos($text,"Proviso");
        $result_string1 = substr("$text",$position_num1+8,8);
        $regfinal= preg_replace('/\s+/', '', $result_string1);
        $position_num2= strpos($text,"trading as");
        if($position_num2)
        {
          $result_string2 = substr("$text",$position_num2+11);
          $arr = explode("\n", $result_string2);
          $cname=$arr[0];
          $cadd=$arr[1];
          $cadd1=$arr[2];
      ...
      {
          $position_num4= strpos($text,"Trading as");
          if($position_num4)
          {
             $result_string2 = substr("$text",$position_num4+11);
             $arr = explode("\n", $result_string2);
             $cname=$arr[0];
             $cadd=$arr[1];
             $cadd1=$arr[2];
         ...
         {
             $position_num5= strpos($text,"Proviso");
             $result_string2 = substr("$text",$position_num5+30);
             $arr = explode("\n", $result_string2);
             $cname=$arr[1];
             $cadd=$arr[2];
             $cadd1=$arr[3];
     ...
     $position_num6 = strpos($text,"Advertised");
     $result_string6 = substr("$text",$position_num-9);
     $arr = explode("\n", $result_string6);
	//print_r($arr);
     if(substr("$arr[2]",0,10)=="Advertised" || substr("$arr[4]",0,10)=="Advertised")
     {
      
      
      shell_exec("convert ".$imgf."-00".$n.".ppm ".$imgf."-00".$n.".jpg" );
  ...
  //  
    //echo $result_string1 ." ".$result_string6;
  $caddn = preg_replace('/\s+/', '', $cadd);
  $caddchck=substr($caddn,-1);
  $caddn1 = preg_replace('/\s+/', '', $cadd1);
  $caddchck1=substr($caddn1,-1);
    //echo $caddchck1;
    //echo $caddchck;
  if($caddchck==":" || $caddchck==",")
  {
      $caddfinal=$cadd." ".$cadd1;
  }
  else if(is_numeric($caddchck1))
  {
  ...
  else
  {
	...
	//echo "CLASS: ".$result_string." REG NO: ".$result_string1."<br>DETAILS: <br>".$cname."<br>".$caddfinal."<br>"."LOGO: ".$logo."<br><br>";
	/*$data = array('reg' => mysql_real_escape_string($result_string1),
              'class' => mysql_real_escape_string($result_string),
              'cn' => mysql_real_escape_string($cname),
              'ca' =>mysql_real_escape_string($caddfinal),
              'lo' =>mysql_real_escape_string($logo)
              );*/


          //   $this->load->model('modelmera',$data);
            // $this->db->query("INSERT INTO pdfdetails VALUES('$regfinal','$result_string','$cname','$caddfinal','$logo')");
$datatoins=array('reg'=>$regfinal,'cls'=>$result_string,'cn'=>$cname,'ca'=>$caddfinal,'logo'=>$logo);
$this->load->model('finalmodel');
$this->finalmodel->insertdb($datatoins);
}
